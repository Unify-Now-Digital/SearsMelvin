...
try {
  ...
  assert.equal(partnerHtml.includes('id="wizardRequestPayment"'), false);
  assert.equal(partnerHtml.includes('state.wizardStep===5?"Create quote record"'), true);
  assert.equal(partnerHtml.includes('data-view="permits"'), true);
  assert.equal(partnerHtml.includes('data-view="payments"'), true);
  assert.equal(partnerHtml.includes('data-view="proofs"'), true);
  assert.equal(partnerHtml.includes('data-view="installation"'), true);
  assert.equal(partnerHtml.includes("Recognition never constitutes customer approval"), true);
  assert.equal(partnerHtml.includes("Open this permit in Permit desk"), true);
  assert.equal(partnerHtml.includes("action:\"update-permit\""), true);
  assert.equal(partnerHtml.includes("Direct stage editing is disabled"), false);
  assert.equal(partnerHtml.includes("Permit status: Select form"), false);
  assert.equal(partnerHtml.includes('spineLabel||"Match form"'), true);
  assert.equal(partnerHtml.includes("receive signed from the correct person"), true);
  assert.equal(partnerHtml.includes("complete memorial and our details"), true);
  assert.equal(partnerHtml.includes("Do not collapse this to with-family"), true);
  assert.equal(partnerHtml.includes("does not create or send invoices"), true);
  assert.equal(partnerHtml.includes("Spec-before-pay is unlocked"), true);
  assert.equal(partnerHtml.includes('["confirmed","Confirmed"],["needs_action","Needs action"],["waiting","Waiting"],["quotes","Quotes"],["complete","Complete"],["all","All"]'), true);
  assert.equal(partnerHtml.includes("Approve proof"), true);
  assert.equal(partnerHtml.includes("Request payment is a separate action on the quote"), true);
