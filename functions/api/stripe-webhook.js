...
async function handlePaymentSucceeded(env, pi) {
  ...
  // 1. Mark order as deposit paid in Supabase (non-critical)
  if (env.SUPABASE_URL && env.SUPABASE_SERVICE_KEY && email) {
    try {
      await fetch(
        `${env.SUPABASE_URL}/rest/v1/orders?customer_email=eq.${encodeURIComponent(email)}&order=created_at.desc&limit=1`,
        {
          method:  "PATCH",
          headers: {
      ...
      console.error("Supabase deposit update failed:", err);
    }
  }
