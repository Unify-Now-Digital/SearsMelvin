...
--
-- PAYLOAD CONTRACT — must match handleQuoteRequest in functions/api/submit.js:
--   {
--     organization_id, email, first_name, last_name, name, phone,
--     source_page, message,
--     location,        -- cemetery free-text (or chosen cemetery's label)
--     cemetery_id,     -- optional; uuid of a chosen cemetery
--     edit_token,      -- generated by the worker and emailed to the customer
--     product          -- the full configurator object (jsonb)
--   }
-- The worker supplies edit_token (the customer's "edit your quote" link uses
-- it), so the function MUST persist that value rather than mint its own.
--
-- Mirrors the previous JS orchestration:
--   • people     — deduped by email (lower-cased, GLOBALLY unique). Never sets
--                  is_customer; on an existing row refreshes name/phone but NOT
--                  organization_id (a contact registered under another tenant
--                  keeps theirs).
--   • orders     — order_type='quote'; product_config stored as a JSON *string*
--                  (text column); resolved cemetery_id set on the order too.
--   • enquiries  — channel='quote'; details stored as JSONB; linked via order_id.
--   • cemetery   — supplied cemetery_id, else best-effort name match against
--                  `location` (exact ilike → prefix → contains), else null.
--                  Unscoped by org, matching the existing JS lookup.
--
-- Returns the new ids as jsonb. Uses %TYPE anchors + ::text id comparisons so it
-- works whether id columns are uuid or bigint, and a select-then-insert/update
-- person upsert with a unique_violation fallback so it does not depend on the
-- name of the email unique index.
...
begin
  ...
  -- 1. Person upsert (dedupe by globally-unique email). select-then-insert/update
  --    with a unique_violation fallback so it does not depend on the unique
  --    index name. organization_id is never changed for an existing row.
