...
// Stone colour → hex (for the coloured sidebar in emails)
const STONE_COLOURS = {
  "Black Galaxy":    "#1a1a1a",
...
async function handleQuoteRequest(env, data, submittedAt, corsHeaders) {
  ...
  // Resolve stone colour swatch hex
  const stoneHex = STONE_COLOURS[product.colour] || "#8B7355";

  // ── Send branded quote notification to the business (critical) ────────────
  try {
  ...
  // ── Send quote confirmation to the customer (non-critical) ────────────────
  try {
    await sendEmail(env.RESEND_API_KEY, {
      from:    `${BUSINESS_NAME} <${FROM_EMAIL}>`,
      to:      email,
...
async function handleEnquiry(env, data, submittedAt, corsHeaders) {
  ...
  // ── Send notification email to the business (critical) ────────────────────
  try {
    await sendEmail(env.RESEND_API_KEY, {
      from:    `${BUSINESS_NAME} <${FROM_EMAIL}>`,
      to:      BUSINESS_EMAIL,
      ...
      html: `
        <div style="font-family:-apple-system,sans-serif; max-width:600px; color:#1a1a1a; background:#fff; border-radius:8px; overflow:hidden; border:1px solid #E0DCD5;">
          ...
          <div style="padding:24px;">
            ...
            <p style="color:#999; font-size:12px; margin:0 0 20px;">Received ${submittedAt}</p>
  ...
  // ── Send confirmation to the customer (non-critical) ──────────────────────
  try {
    await sendEmail(env.RESEND_API_KEY, {
      from:    `${BUSINESS_NAME} <${FROM_EMAIL}>`,
      to:      email,
      ...
      html: `
        <div style="font-family:-apple-system,sans-serif; max-width:580px; color:#1a1a1a;">
          ...
          <div style="background:#fff; padding:28px 24px; border:1px solid #E0DCD5; border-top:none; border-radius:0 0 8px 8px;">
            ...
            <p style="color:#555; line-height:1.7; margin:0 0 24px;">If you have any urgent questions in the meantime, please call us on <strong>01268 208 559</strong>.</p>
  ...
  // ── Create task in ClickUp Orders list (non-critical) ────────────────────
  try {
    await createClickUpTask(env.CLICKUP_API_KEY, {
...
/**
 * Business notification — Option A: Product card layout
 * Branded header · Coloured swatch sidebar · Customer details block
 */
...
<table width="100%" cellpadding="0" cellspacing="0" style="background:#F5F3F0;padding:24px 0;">
  <tr><td align="center">
    <table width="620" cellpadding="0" cellspacing="0" style="max-width:620px;background:#fff;border-radius:10px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,0.08);">
      ...
      <tr>
        <td colspan="2" style="background:#2C2C2C;padding:18px 28px;">
          <table width="100%" cellpadding="0" cellspacing="0">
            <tr>
              <td><span style="font-family:Georgia,'Times New Roman',serif;font-size:18px;color:#fff;font-weight:normal;">Sears Melvin <span style="opacity:0.55;font-weight:300;">Memorials</span></span></td>
      ...
      <!-- ── Product card (coloured sidebar + details) ── -->
      <tr>
        <td colspan="2" style="padding:20px 28px 0;">
          <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #E0DCD5;border-radius:8px;overflow:hidden;">
            <tr>
              <!-- Coloured stone-colour swatch bar -->
...
/**
 * Customer confirmation email — warm, branded, includes quote summary
 */
    ...
    <table width="580" cellpadding="0" cellspacing="0" style="max-width:580px;background:#fff;border-radius:10px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,0.08);">
      ...
      <tr>
        <td style="background:#2C2C2C;padding:20px 28px;border-radius:10px 10px 0 0;">
          <span style="font-family:Georgia,'Times New Roman',serif;font-size:18px;color:#fff;font-weight:normal;">Sears Melvin <span style="opacity:0.55;font-weight:300;">Memorials</span></span>
        </td>
      </tr>
      ...
      <tr>
        <td style="padding:0 28px 32px;">
          <p style="color:#555;font-size:14px;line-height:1.7;margin:0 0 10px;">
            ...
            <strong style="color:#2C2C2C;">01268 208 559</strong>.
          </p>
...
// ─── ClickUp structured description for quote tasks ──────────────────────────
